...
export default async function CourseLogPage() {
  const admin = await isSuperAdmin();
  if (!admin) redirect("/admin/super-login?redirectTo=/admin/course-log");
    ...
    <div className="min-h-screen">
      <div className="max-w-4xl mx-auto px-4 py-8 flex flex-col gap-5">
        <header className="border-b-2 border-ink pb-4">
          ...
          <h1 className="font-display text-2xl text-ink">코스 아카이브 변경 로그</h1>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b border-line-strong text-left">
                ...
                <th className="px-3 py-2 font-mono-brand text-[10.5px] uppercase text-ink-faint">코스명</th>
            ...
            <tbody>
              {logs.map((log) => {
                let courseName = "";
                try {
                  courseName = JSON.parse(log.courseSnapshot).name ?? "";
                } catch {
                  courseName = "";
                }
                return (
                  <tr key={log.id} className="border-b border-line">
                    ...
                    <td className="px-3 py-2 text-ink">{courseName}</td>
